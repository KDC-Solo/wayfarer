...
- `App.tsx` is tab-based (`Nav.tsx`, `TabId` = company/oracle/journey/fellowship/templates), not a flat vertical stack of every panel — company management + rolling live under "Company," oracle + tables under "Oracle," and so on. When a journey or Fellowship phase begins, `App.tsx` auto-switches to that tab (see `handleBeginJourney`/`handleBeginFellowshipPhase`) since that's where the user needs to be next.
